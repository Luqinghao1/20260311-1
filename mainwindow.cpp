 ...
 * 4. 初始化多数据拟合标签页管理器（WT_MultidataFittingPage）以替换原有的预测界面，实现多数据功能入口。
 */
...
void MainWindow::init()
{
    ...
    if (ui->pagePrediction && ui->verticalLayoutPrediction) {
        WT_MultidataFittingPage* multiPage = new WT_MultidataFittingPage(ui->pagePrediction);
...
void MainWindow::onProjectOpened(bool isNew)
{
    ...
    if (m_DataEditorWidget) {
        ...
        WT_MultidataFittingPage* multiPage = ui->pagePrediction->findChild<WT_MultidataFittingPage*>("MultiDataFittingPage");
        if (multiPage) {
